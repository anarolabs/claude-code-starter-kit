...
Usage:
    ...
    python3 docs_operations.py --create-md --title "Document Title" --file /path/to/content.md
    ...
    python3 docs_operations.py --read DOC_ID
...
def create_document(title: str, content: str = None, folder_id: str = None):
...
def create_from_markdown(title: str, markdown_file: str, folder_id: str = None):
    """
...
def batch_update(doc_id: str, requests_json: str):
...
def update_from_markdown(doc_id: str, markdown_file: str):
    """
...
def main():
    ...
    if args.create:
        ...
        create_document(args.title, args.content, args.folder)

    elif args.create_md:
        if not args.title or not args.file:
            parser.error("--create-md requires --title and --file")
        create_from_markdown(args.title, args.file, args.folder)

    elif args.read:
        read_document(args.read)

    elif args.structure:
        get_structure(args.structure)

    elif args.append:
        if not args.text:
            parser.error("--append requires --text")
        append_text(args.append, args.text)

    elif args.batch:
        if not args.requests:
            parser.error("--batch requires --requests")
        batch_update(args.batch, args.requests)

    elif args.update_md:
        if not args.file:
            parser.error("--update-md requires --file")
        update_from_markdown(args.update_md, args.file)
