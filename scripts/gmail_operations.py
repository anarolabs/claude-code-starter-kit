...
- "•" character with <br> between items for bullets (NOT <ul>/<li>)
- No extra padding or ugly spacing

Usage:
    # Search emails
    python3 gmail_operations.py --search "from:client@company.com after:2025-01-01" [--max 10]

    # Read email
    python3 gmail_operations.py --read MESSAGE_ID
    ...
    # List labels
    python3 gmail_operations.py --labels
"""
import argparse
import base64
import json
...
def _fetch_reply_headers(message_id: str) -> dict:
...
def list_labels():
    ...
    try:
        ...
        print(json.dumps({"labels": output, "count": len(output)}, indent=2))
...
def main():
    ...
    if args.search:
        search_emails(args.search, args.max)

    elif args.read:
        read_email(args.read)
        ...
        if args.reply_to:
            try:
                reply_ctx = _fetch_reply_headers(args.reply_to)
            except Exception as e:
